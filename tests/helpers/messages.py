...
def strip_expected_updates(message: Message) -> Message:
    video_frame = message.as_video_frame() if message.is_video_frame() else None
    if not video_frame:
        return message
    ...
    video_frame = VideoFrame(
        ...
        content=video_frame.content,
        codec=video_frame.codec,
    )
    return Message.video_frame(video_frame)
